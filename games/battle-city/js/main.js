...
var ctx;         // 2d stage canvas context
var wallCtx;     // map/wall canvas context
var grassCtx;    // grass layer canvas context
var tankCtx;     // tank canvas context
var overCtx;     // game-over overlay canvas context
var menu = null; // menu object
var stage = null;// stage transition object
var map = null;  // map object
var player1 = null; // player 1
var player2 = null; // player 2
var prop = null;
var enemyArray = [];  // enemy tanks
var bulletArray = []; // bullet array
var keys = [];        // currently pressed keys
var crackArray = [];  // explosion animation array

var gameState = GAME_STATE_MENU; // default: menu state
var level = 1;
var maxEnemy = 20;        // total number of enemy tanks
var maxAppearEnemy = 5;   // max enemies on screen at once
var appearEnemy = 0;      // number of enemies that have spawned
var mainframe = 0;
...
function initObject(){
	...
	player2.offsetX = 128; // player 2 sprite is offset 128px from player 1
	player2.x = 256 + map.offsetX;
	player2.y = 385 + map.offsetY;
	appearEnemy = 0;     // reset spawned enemy count
	enemyArray = [];     // enemy tanks
	bulletArray = [];    // bullet array
	keys = [];           // pressed keys
	crackArray = [];     // explosion animations
	isGameOver = false;
	overX = 176;
	overY = 384;
...
function addEnemyTank(){
	...
	// update the enemy count shown on the right side
...
function gameOver(){
	...
	if(overY <= parseInt(map.mapHeight/2)){
		initObject();
		// single player: disable player 2
		if(menu.playNum == 1){
			player2.lives = 0;
		}
